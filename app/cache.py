"""Redis cache layer (Milestone C, L1) — destination research + geocodes.
...
Posture (per the plan): **caching is an optimisation, never a hard dependency.**
...
everything at once):
  nomad:research:{ver}:{slug}   → JSON list of ResearchDiscovery   (TTL: ~45d)
...
async def get_cached_research(destination: str) -> list[ResearchDiscovery] | None:
    """Return the cached discovery pool for a destination, or None on miss/error."""
    client = _get_client()
    if client is None:
        return None
    try:
        raw = await client.get(_research_key(destination))
        if not raw:
            return None
        data = json.loads(raw)
